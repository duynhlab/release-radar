 ...
 * rather than dropping the attribute. Two consequences we have to handle here:
 *
 *   1. `[x](javascript:alert(1))` becomes `<a href="">x</a>` — a dead link that
 *      still passes a naive `not.toContain("javascript:")` assertion. Treat ""
 ...
 *
 * We also drop mailto: and tel:, which core permits — a release note has no
...
export function isAllowedNoteImage(src: string): boolean {
  ...
  try {
    ...
    return (
      url.protocol === "https:" && ALLOWED_NOTE_IMAGE_HOSTS.includes(url.hostname)
    );
  } catch {
    return false;
  }
